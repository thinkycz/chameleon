...
class BulkImageUploadController extends Controller
{
    public function upload(Request $request)
    {
        ...
        $filename = $this->extractFilename($file, $request->get('separator'));
    ...
    private function extractFilename(UploadedFile $file, $separator = '|')
    {
        return collect(explode($separator, pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME)))->first();
    }
}
